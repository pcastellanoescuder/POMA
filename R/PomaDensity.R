...
#' @param group Groupping factor for the plot. Options are c("samples", "features"). Option "samples" (default) will create a density plot for each group and option "features" will create a density plot of each variable.
  ...
  if(group == "samples"){
    ...
    if (is.null(feature_name)){
      ...
      data %>%
        ...
        geom_density(alpha = 0.4) +
        theme_minimal() +
        xlab(ifelse(length(feature_name) > 1, paste0(feature_name, collapse = ", "), feature_name))
