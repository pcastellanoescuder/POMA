...
The **MSnbase** Bioconductor package provides a well defined computational data structures to represent mass spectrometry (MS) experiment data types [@MSnbase][@Biobase]. Since data structures can mean a marked improvement in data analysis, **POMA** functions use **MSnSet** objects from **MSnbase** package, allowing the reusability of existing methods for this class and contributing to the improvement of robust and reproducible workflows.    
...
_Note that the object has been updated with outlier information._    
...
```{r}
# alpha = 1 for Lasso
PomaLasso(pre_processed, alpha = 1)$coefficientPlot
```

```{r}
# alpha = 0 for Ridge
PomaLasso(pre_processed, alpha = 0)$coefficientPlot
```
